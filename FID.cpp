...
FID::FID(const uint8_t* BIT) {
	...
	blockLarge[0] = 0;
	for (size_t i = 0, largeIdx = 0; i < originalBitLength and largeIdx < largeLength;  ++i)
	{
		if (i % largeSize == 0) {
			++largeIdx;
	...
	for (size_t i = 0, smallIdx = 0; i < originalBitLength and smallIdx < smallLength; ++i)
	{
		if (i % smallSize == 0)
		{
			if (i % largeSize == 0)
...
uint16_t FID::rank(int pos, int digit) {
	...
	if (digit == 0) result = pos - result;
	return result;
}

uint16_t FID::select(int idx, int digit)
{
	uint16_t lower = 0, upper = originalBitLength;
	while (upper - lower > 1)
	{
